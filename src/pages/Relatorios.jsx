...
export default function Relatorios({ user }) {
  ...
  async function carregarDados() {
    try {
      ...
      const biometriaFiltrada =
        filtrarTanque(biometria)

      const mortalidadeFiltrada =
        filtrarTanque(mortalidade)

      const custosFiltrados =
        filtrarTanque(custos)

      const vendasFiltradas =
        filtrarTanque(vendas)
      ...
      if (tanqueSelecionado === "todos") {
        ...
        biomassa =
          Number(ultima?.biomassa || 0)

        peixes =
          Number(ultima?.quantidade || 0)
      }

      const totalMortalidade =
        mortalidadeFiltrada.reduce(
          (acc, item) =>
      ...
      setBiometrias(biometriaFiltrada || [])
    } catch (erro) {
      console.log(erro)
    }
  }
  ...
  function gerarExcel() {
    const dados = [
      ...
      {
        Indicador: "Biomassa",
        Valor: relatorio.biomassa,
      },
      {
        Indicador: "Peixes",
        Valor: relatorio.peixes,
      },
      {
        Indicador: "Mortalidade",
        Valor: relatorio.mortalidade,
      },
      {
        Indicador: "Custos",
        Valor: relatorio.custos,
      },
      {
        Indicador: "Vendas",
        Valor: relatorio.vendas,
      },
      {
        Indicador: "Lucro",
        Valor: relatorio.lucro,
      },
      {
        Indicador: "Ração",
        Valor: relatorio.racao,
      },
      {
        Indicador: "RCA",
        Valor: relatorio.rca,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card
          titulo="Biomassa"
          valor={`${moeda(relatorio.biomassa)} kg`}
          cor="bg-blue-100"
        />
        ...
        <Card
          titulo="Custos"
          valor={`R$ ${moeda(relatorio.custos)}`}
          cor="bg-yellow-100"
        />

        <Card
          titulo="Vendas"
          valor={`R$ ${moeda(relatorio.vendas)}`}
          cor="bg-purple-100"
        />

        <Card
          titulo="Lucro"
          valor={`R$ ${moeda(relatorio.lucro)}`}
          cor="bg-emerald-100"
        />

        <Card
          titulo="Ração"
          valor={`${moeda(relatorio.racao)} kg`}
          cor="bg-orange-100"
        />

        <Card
          titulo="RCA"
          valor={moeda(relatorio.rca)}
          cor="bg-cyan-100"
        />
