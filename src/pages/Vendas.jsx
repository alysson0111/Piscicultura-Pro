...
export default function Vendas({ user }) {
  ...
  const totalVenda =
    Number(peso || 0) * Number(valorKg || 0)
  ...
  async function salvarVenda(e) {
    ...
    const payload = {
      ...
      cliente,
      peso: Number(peso),
      valor_kg: Number(valorKg),
      valor_total: Number(totalVenda),
      data_venda: dataVenda,
    }

    const query = editando
      ? supabase.from("vendas").update(payload).eq("id", editando)
      : supabase.from("vendas").insert([payload])
    ...
    alert(editando ? "Venda atualizada!" : "Venda salva!")
  }
  ...
  function editarVenda(item) {
    ...
    setPeso(item.peso || "")
    setValorKg(item.valor_kg || "")
    setDataVenda(item.data_venda || "")
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div>
          <label className="font-bold">Peso Total (kg)</label>
          <input
            type="number"
            step="0.01"
            value={peso}
        ...
        <div>
          <label className="font-bold">Valor por Kg</label>
          <input
            type="number"
            step="0.01"
            value={valorKg}
        ...
        <div>
          <label className="font-bold">Data da Venda</label>
          <input
            type="date"
            value={dataVenda}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b bg-slate-100">
              ...
              <th className="p-3 text-left">Valor/Kg</th>
